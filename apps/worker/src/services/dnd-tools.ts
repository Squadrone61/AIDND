    ...
    parameters: {
      ...
      properties: {
        spell_name: {
          type: "string",
          description:
            "Spell name, e.g. 'fireball', 'cure wounds', 'shield'",
        },
      },
      required: ["spell_name"],
    },
    ...
    parameters: {
      ...
      properties: {
        section: {
          type: "string",
          description:
            "Rule section index, e.g. 'casting-a-spell', 'the-order-of-combat', 'damage-and-healing', 'saving-throws'",
        },
      },
      required: ["section"],
    },
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "Partial spell name to search for, e.g. 'fire', 'heal', 'shield'",
        },
      },
      required: ["query"],
    },
  ...
  try {
    switch (name) {
      case "lookup_spell": {
        ...
        const spell = await lookupSpell(spellName, kv);
      ...
      case "lookup_monster": {
        ...
        const monster = await lookupMonster(monsterName, kv);
        if (!monster) return { content: `Monster "${monsterName}" not found in the D&D 5e SRD. It may be from a published sourcebook not in the SRD, or the name may be misspelled. Use your training knowledge for this creature.`, isError: false };
      ...
      case "lookup_condition": {
        ...
        const condition = await lookupCondition(conditionName, kv);
      ...
      case "lookup_rule": {
        ...
        const rule = await lookupRule(section, kv);
        if (!rule) return { content: `Rule section "${section}" not found. Try a different section index (e.g. 'casting-a-spell', 'the-order-of-combat', 'damage-and-healing').`, isError: false };
      ...
      case "search_spells": {
        ...
        const results = await searchSpells(query, kv);
